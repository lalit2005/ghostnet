...
class GhostNetBackbone(nn.Module):
    def __init__(self, width_mult=1.0):
        ...
        # config obtained from table 1 of the paper
        self.config = [
        ...
        for k, exp, c, se, s in self.config:
            ...
            hidden_c = _make_divisible(exp * width_mult)
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        features = {}
        # get all the features at strides 8, 16, 32 for fpn
        # indices correspond to the bottleneck layers in self.layers
...
def get_ghost_detection_model(num_classes):
    backbone = GhostNetBackbone()
    
    # fpn (feature pyramid network)
    # in_channels_list matches the outputs of ghostnet at strides 8, 16, 32
    ...
    anchor_generator = AnchorGenerator(sizes=((32,), (64,), (128,)), aspect_ratios=((0.5, 1.0, 2.0),) * 3)
    roi_pooler = MultiScaleRoIAlign(featmap_names=['0', '1', '2'], output_size=7, sampling_ratio=2)
    
    model = FasterRCNN(full_backbone, num_classes=num_classes, rpn_anchor_generator=anchor_generator, box_roi_pool=roi_pooler)
    return model
...
if __name__ == "__main__":
    ...
    print("Loading COCO dataset (this might take a moment)...")
    dataset = CocoDetectionWrapper(root=train_dir, annFile=train_ann, transform=transform)
    ...
    if os.path.exists(weights_path):
        print(f"Found pre-trained weights: {weights_path}")
        try:
            # loading the imagenet pretrained weights, as mentioned in the paper
            state_dict = torch.load(weights_path, map_location=device)
            # model -> backbone -> body -> layers
            model.backbone.body.load_state_dict(state_dict, strict=False)
            print(">> loaded pre-trained backbone weights (partial/strict=false)")
        except Exception as e:
            print(f">> Warning: Failed to load weights ({e}). Training from scratch.")
    else:
        print(">> No pre-trained weights found ('ghostnet_1x_imagenet.pth'). Training from scratch.")
    ...
    try:
        for epoch in range(num_epochs):
            ...
            # this saves the state - the .pth file after every epoch
            torch.save(model.state_dict(), f"ghost_fasterrcnn_epoch_{epoch+1}.pth")

    except KeyboardInterrupt:
        print("\n\n[!] Training interrupted by user (Ctrl+C).")
        print("Saving current model state...")
        torch.save(model.state_dict(), "ghost_fasterrcnn_interrupted.pth")
        print("Model saved to 'ghost_fasterrcnn_interrupted.pth'. Exiting.")
